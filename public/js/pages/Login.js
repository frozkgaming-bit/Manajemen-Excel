...
export default function Login() {
    return `
                ...
                <div class="bg-white/95 backdrop-blur-sm rounded-2xl border border-slate-200/90 shadow-[0_20px_50px_rgba(15,23,42,0.08)] p-8 sm:p-10 transition-all duration-300 hover:shadow-[0_24px_60px_rgba(15,23,42,0.12)]">
                    ...
                    <form action="#" class="space-y-5" method="POST" onsubmit="event.preventDefault();">
                        ...
                        <div data-purpose="input-group-username">
                            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-700 mb-1.5" for="loginUsername">
                                Username
                            </label>
                        ...
                        <div data-purpose="input-group-password">
                            ...
                            <div class="relative rounded-lg shadow-sm"><div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                                <!-- Lock Icon -->
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            ...
                            <input class="block w-full pl-10 pr-10 py-2.5 bg-slate-50/60 border border-slate-300 rounded-lg text-sm text-slate-900 placeholder-slate-400 tracking-wider focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition-colors" id="loginPassword" name="password" placeholder="Masukkan password Anda" required="" type="password" value="••••••">
                            <button aria-label="Tampilkan password" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600 transition-colors focus:outline-none" id="toggle-password-btn" onclick="const pw=document.getElementById('password'); const isPw=pw.type==='password'; pw.type=isPw?'text':'password'; this.querySelector('.eye-open').classList.toggle('hidden', isPw); this.querySelector('.eye-closed').classList.toggle('hidden', !isPw);" type="button">
                                <svg class="h-4 w-4 eye-open" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                <svg class="h-4 w-4 eye-closed hidden" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <!-- END: MainContent -->
        <!-- BEGIN: Footer -->
        <footer class="relative z-10 py-5 text-center text-xs text-slate-400 border-t border-slate-200/60 bg-white/40 backdrop-blur-xs">
            <p class="">© 2026 BOS EDS • Sistem Informasi & Validasi Data Pertanahan</p>
        </footer>
        <!-- END: Footer -->
    `;
}
...
export function initLoginPage() {
    // Import auth module dynamically to initialize login handlers
...
export function initLoginPageScript() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            if (!usernameInput.value.trim() || !document.getElementById('loginPassword').value) {
